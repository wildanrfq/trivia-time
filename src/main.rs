...
fn decode(raw: &json::JsonValue) -> String {
...
fn main() -> Result<()> {
    ...
    let question = decode(&data["question"]);
    let category = decode(&data["category"]);
    let correct = decode(&data["correct_answer"]);
    let letters = vec![String::from("A"), String::from("B"), String::from("C"), String::from("D")];
    let mut answers = Vec::new();
    answers.push(decode(&data["correct_answer"]));
    answers.push(decode(&data["incorrect_answers"][0]));
    answers.push(decode(&data["incorrect_answers"][1]));
    answers.push(decode(&data["incorrect_answers"][2]));
    ...
    let answerr: &str = &answer;
    if letters.contains(&answer) {
        if final_[answerr] == correct {
